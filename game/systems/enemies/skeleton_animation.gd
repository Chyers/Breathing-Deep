...
func _physics_process(delta: float) -> void:
	...
	# While attacking, do nothing else (must come after the timer tick)
	if is_attacking:
	...
	if dist <= stop_distance:
		...
		if attack_timer <= 0:
			attack()
			return  # Exit before sprite.play() can override "attack"
	...
	# Only reached when not attacking
...
func _on_animation_finished() -> void:
	match sprite.animation:
		"attack":
			is_attacking = false
			has_dealt_damage = false  # Reset for next swing
